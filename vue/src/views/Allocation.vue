...
<!--      <el-button class="ml-5" type="primary" @click="handleAdd" >新增<i class="el-icon-circle-plus-outline"></i></el-button>-->
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">>
      ...
      <el-table-column prop="allocation" label="调拨流程" >
        <template slot-scope="scope">
          <el-tag type="danger" v-if="scope.row.allocation === 0">未处理</el-tag>
          <el-tag type="primary" v-if="scope.row.allocation === 1">待处理</el-tag>
          <el-tag type="success" v-if="scope.row.allocation === 2">已处理</el-tag>
        </template>
      </el-table-column>
...
<!--      </el-table-column>-->
      ...
      <el-table-column prop="adminAllocationName" label="审批" ></el-table-column>
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">

          <!--                编辑与删除按钮-->
<!--          <el-button type="success" @click="handleEdit(scope.row)" v-if="scope.row.allocation === 1"  >编辑<i class="el-icon-edit-outline"></i></el-button>-->
          ...
          >
            <el-button type="danger" slot="reference" v-if="scope.row.allocation === 1
          ...
          <el-button class="ml-5" type="primary" @click="commitCheck(scope.row.id)" v-if="scope.row.allocation === 1
...
export default {
  ...
  methods:{
    load(){ //请求分页查询数据，加载页面
      this.request.get("assets/page/allocation",{
        params:{
    ...
    commitCheck(id){  //调拨流程转向处置流程
      this.request.post("/assets/updateManagement/" + id + "/" + this.user.id) .then(res => {
        if (res.code === '200') {

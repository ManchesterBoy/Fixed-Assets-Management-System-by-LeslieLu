      ...
      <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">>
        ...
        <el-table-column prop="purchase" label="购置流程" >
          <template slot-scope="scope">
            ...
            <el-tag type="primary" v-if="scope.row.purchase === 1">待处理</el-tag>
        ...
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
                     ...
                     <!--                编辑与删除按钮-->
            <el-button type="success" @click="handleEdit(scope.row)" v-if="scope.row.purchase === 1" >编辑<i class="el-icon-edit-outline"></i></el-button>
            ...
            >
              <el-button type="danger" slot="reference" v-if="scope.row.purchase === 1
...
<!--            <el-button class="ml-5" type="primary" @click="lookCourse(scope.row.courses)" v-if="scope.row.role === 'ROLE_TEACHER'">查看教授课程<i class="el-icon-document"></i></el-button>-->
            <el-button class="ml-5" type="primary" @click="commitCheck(scope.row.id)" v-if="scope.row.purchase === 1
      ...
      <el-dialog title="业务信息" :visible.sync="dialogFormVisible" width="30%" >
        <el-form label-width="80px" size="mid" :model="form" :rules="rules" ref="pass">
          <el-form-item label="事件名称" prop="name">
            <el-input v-model="form.name" autocomplete="off" placeholder="请输入事件名称"></el-input>
          </el-form-item>
          <el-form-item label="具体内容" prop="content">
            <el-input type="textarea" v-model="form.content" autocomplete="off" placeholder="请输入具体内容"></el-input>
          </el-form-item>
          ...
          <el-form-item label="购置申请" prop="purchase">
...
export default {
  ...
  data(){
    return{
      ...
      rules: {
        ...
        userId: [
          { required: true, message: '请选择当前用户', trigger: 'change' },
        ],
        purchase: [
      ...
      asset: localStorage.getItem("asset") ? JSON.parse(localStorage.getItem("asset")) : {},
    }
  },
  created(){
    //请求分页查询数据
  ...
  methods:{
    load(){ //请求分页查询数据，加载页面
      this.request.get("assets/page/purchase",{
        params:{
        ...
        console.log(res)
        localStorage.setItem("assets",JSON.stringify(res.data.records))   //存储固定资产信息到浏览器
    ...
    commitCheck(id){  //购置流程转向验收流程
      this.request.post("/assets/updateAccept/" + id + "/" + this.user.id) .then(res => {
        if (res.code === '200') {
